...
import DOBillChart from './DOBillChart';
...
class DOBillWrapper extends React.Component {
	constructor(props) {
		super(props);
	...
	componentDidMount () {
		var s3 = new AWS.S3();
		s3.getObject(
			{ Bucket: "divdata", Key: "digitalocean.csv" },
			this.handleS3Download
		);
	}
	render () {
		return (
			<DOBillChart data={this.state.data} />
		)
	}
}
export default DOBillWrapper
