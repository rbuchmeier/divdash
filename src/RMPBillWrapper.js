...
import RMPBillChart from './RMPBillChart';
...
class RMPBillWrapper extends React.Component {
	constructor(props) {
		super(props);
	...
	componentDidMount () {
		var s3 = new AWS.S3();
		s3.getObject(
			{ Bucket: "divdata", Key: "rmp_bills.csv" },
			this.handleS3Download
		);
	}
	render () {
		return (
			<RMPBillChart data={this.state.data} />
		)
	}
}
export default RMPBillWrapper
